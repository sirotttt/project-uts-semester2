...
<?php 
   ...
   if($_proses == "Simpan"){
    // data baru
    $sql = "INSERT INTO pesanan (nama_pelanggan,alamat_pelanggan,motor_id,
    quantity) VALUES (?,?,?,?)";
   }else if($_proses == "Update"){
    $ar_data[]=$_POST['idedit'];// ? 8
    $sql = "UPDATE pesanan SET nama_pelanggan=?,alamat_pelanggan=?,motor_id=?,
    quantity=? WHERE id=?";
   }
   if(isset($sql)){
    $st = $dbh->prepare($sql);
